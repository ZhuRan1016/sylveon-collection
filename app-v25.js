...
async function init(){
  db=await fetch('master-v25.json?v=2501',{cache:'reload'}).then(r=>r.json());
  cards=db.cards||[];
  cards.forEach(c=>{ if(imageCache[c.id]) c.image_url=imageCache[c.id]; });
  populateFilters();
  bindEvents();
  render();
  if('serviceWorker' in navigator) navigator.serviceWorker.register('service-worker-v25.js?v=2501');
}
