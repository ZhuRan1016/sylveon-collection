...
async function init(){
  db=await fetch('master-v28.json?v=2801',{cache:'reload'}).then(r=>r.json());
  cards=db.cards||[];
  cards.forEach(c=>{ if(imageCache[c.id]) c.image_url=imageCache[c.id]; });
  populateFilters();
  bindEvents();
  render();
  if('serviceWorker' in navigator) navigator.serviceWorker.register('service-worker-v28.js?v=2801');
}
