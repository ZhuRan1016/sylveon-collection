...
async function init(){
  db=await fetch('master-v23.json?v=2301',{cache:'reload'}).then(r=>r.json());
  cards=db.cards||[];
  cards.forEach(c=>{ if(imageCache[c.id]) c.image_url=imageCache[c.id]; });
  populateFilters();
  bindEvents();
  render();
  resolveAllMissingImages();
  if('serviceWorker' in navigator) navigator.serviceWorker.register('service-worker-v23.js?v=2301');
}
