...
async function init(){
  db=await fetch('master-v27.json?v=2701',{cache:'reload'}).then(r=>r.json());
  cards=db.cards||[];
  cards.forEach(c=>{ if(imageCache[c.id]) c.image_url=imageCache[c.id]; });
  populateFilters();
  bindEvents();
  render();
  if('serviceWorker' in navigator) navigator.serviceWorker.register('service-worker-v27.js?v=2701');
}
